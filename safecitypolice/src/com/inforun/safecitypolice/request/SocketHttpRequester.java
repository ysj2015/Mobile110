...
public class SocketHttpRequester { 
	 ...
	 *  * @param params 请求参数 key为参数名,value为参数值 * @param file 上传文件 */
	...
	{ 
		...
		String accept = "Accept: image/gif, image/jpg, image/pjpeg, image/pjpeg, video/mpeg4, application/x-shockwave-flash, "
		...
		String session = "Cookie: JSESSIONID=" + SessionManager.getInstance().getSessionId()+ "\r\n";
		...
		System.out.println("webserver"+reader.readLine());
	...
	public static boolean post(String path, Map<String,String> params, String sessionId) throws Exception{
		final String BOUNDARY = "---------------------------7da2137580612"; //数据分隔线 	
		final String endline = "--" + BOUNDARY + "--\r\n";//数据结束标志
		StringBuilder textEntity = new StringBuilder();
		for (Map.Entry<String,String> entry : params.entrySet()) {
		...
		int dataLength = textEntity.toString().getBytes().length + endline.getBytes().length;
		URL url = new URL(path);
		int port = url.getPort()==-1 ? 80 : url.getPort(); 
		Socket socket = new Socket(InetAddress.getByName(url.getHost()), port); 
		//socket.getSendBufferSize()
		...
		String host = "Host: "+ url.getHost() +":"+ port +"\r\n"; 
		outStream.write(host.getBytes());
		String session = "Cookie: JSESSIONID=" + sessionId + "\r\n";
		outStream.write(session.getBytes());
		//写完HTTP请求头后根据HTTP协议再写一个回车换行
		outStream.write("\r\n".getBytes()); 
		//把所有文本类型的实体数据发送出来 
